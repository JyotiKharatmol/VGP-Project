...
public class Filter_Country_Cell_Department extends NavigateToEmployeeDirectoryPage 

{
	...
	public void Clear() throws IOException
	{
		...
		new WebDriverWait(driver, Duration.ofSeconds(50)).until(ExpectedConditions.visibilityOfAllElements(e.TableRows()));

		int rowSize = e.TableRows().size();
		...
		new WebDriverWait(driver,Duration.ofSeconds(20)).until(ExpectedConditions.elementToBeClickable(e.DepartmentFilter()));
		e.DepartmentFilter().click();
		
		new WebDriverWait(driver, Duration.ofSeconds(10)).until(ExpectedConditions.visibilityOf(e.SelectDepartmentAdmin()));
		...
		List<WebElement> DepartCol = driver.findElements(By.xpath("//tbody/tr/td[5]"));
		
		for(int i=0; i<DepartCol.size();i++)
		{
			if(DepartCol.get(i).getText().equals("Admin"))
			{
				Assert.assertTrue(true);
			}
			else
		...
		e.Clear().click();
		Assert.assertEquals(e.TableRows().size(), rowSize);
